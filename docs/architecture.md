...
- public Markdown CRDT connections are still counted as anonymous note viewers in a separate
  ephemeral service; this exposes counts without turning guests into collaborators
...
- authenticated note presence may include `anonymousViewerCount` for public website viewers of the
  same note, but those guests are never included in the authenticated `viewers` array
